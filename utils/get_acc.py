...
def get_acc(pred, true, logger):
    ...
    for step in range(pred.shape[0]):
        ...
        for idx in range(pred.shape[-1]):
            pred_node, true_node = pred[step,:,idx].reshape(-1), true[step,:,idx].reshape(-1)
            ...
            accH = sum(true_cls[high_idx]==pred_cls[high_idx])/len(true_cls[high_idx]) if high_idx else 0
        ...
        acc_step_H, acc_step_N, acc_step_L = \
            list(filter(lambda x:x!=0, acc_step_H)), list(filter(lambda x:x!=0, acc_step_N)), list(filter(lambda x:x!=0, acc_step_L))
        ...
        logger.info(f'Horizon {step+1:02d} {sum(acc_step)/len(acc_step):.4f}|{acc_step_H_:.4f}|{acc_step_N_:.4f}|{acc_step_L_:.4f}')                              
        
    acc_all_H, acc_all_N, acc_all_L = list(filter(lambda x:x!=0, acc_all_H)), list(filter(lambda x:x!=0, acc_all_N)), list(filter(lambda x:x!=0, acc_all_L))    
